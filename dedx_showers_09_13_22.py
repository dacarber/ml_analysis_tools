...
cfg = yaml.load(open('%s/inference_1252022.cfg' % DATA_DIR, 'r').read().replace('DATA_DIR', DATA_DIR),Loader=yaml.Loader)
...
def compute_shower_dqdx(matched_particles,matched_interaction, r=10, min_segment_size=3):
    ...
    out_diff = true_particles[i].startpoint[0]
    out_false = true_particles[i].startpoint[2]
    return out_photon, out_electron, out, out_true, out_true_photon, out_true_electron, out_diff, out_false
...
for iteration in range(iterations):
    ...
    for entry, index in enumerate(evaluator.index):
        ...
        depo1.extend(dqdx[4])
        depo.extend(dqdx[5])
        #x_startpoint.extend(dqdx[6])
        #z_startpoint.extend(dqdx[7])
#print(f"Mutiple total: {multiples}")
#print(f"False total: {false}")
...
depo = np.array(depo)
#x_startpoint = np.array(x_startpoint)
#z_startpoint = np.array(z_startpoint)
...
pd.DataFrame({'true_p':depo1}).to_csv(f'~/data/data_r{radius}_NuMI_nue_true_photons.csv')
pd.DataFrame({'true_e':depo}).to_csv(f'~/data/data_r{radius}_NuMI_nue_true_electrons.csv')
#pd.DataFrame({'x_start':x_startpoint}).to_csv(f'~/data/data_r{radius}_NuMI_nue_x_startpoint.csv')
#pd.DataFrame({'z_start':z_startpoint}).to_csv(f'~/data/data_r{radius}_NuMI_nue_z_startpoint.csv')
