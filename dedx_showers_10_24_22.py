'''
dedx_showers_10_24_22
Shower dE/dx using true information non-ghost
'''
...
def compute_shower_dqdx(matched_particles,matched_interaction, r=10, min_segment_size=3):
    ...
    for i in range(len(matched_particles)): #Loops over all particles in the matched particle list
        ...
        if(particles[i].pid > 1): #Selects either photon or electron particles
            continue
        p = true_particles[i].asis
        #if (p.pdg_code() == 22 or abs(p.pdg_code()) == 11) and p.creation_process() in ['Decay', 'primary']:
        ...
        true_dist = cdist(true_particles[i].coords_noghost, true_startpoint.reshape(1,-1))
        true_mask = true_dist.flatten() < r
        ...
        true_selected_points = true_particles[i].coords_noghost[true_mask]
        ...
        dq_true = np.sum(true_particles[i].depositions_noghost[true_mask])
            #print(dq)
...
pd.DataFrame({'photons':depo1}).to_csv(f'~/data/data_r{radius}_c2_truenoghost_photons.csv')
pd.DataFrame({'electrons':depo}).to_csv(f'~/data/data_r{radius}_c2_truenogost_electrons.csv')
pd.DataFrame({'reco':collect_dqdx}).to_csv(f'~/data/data_r{radius}_c2_truenoghost_reco.csv')
pd.DataFrame({'true':collect_dqdx_true}).to_csv(f'~/data/data_r{radius}_c2_truenoghost_true.csv')
...
plt.hist(collect_dqdx_true, range=[0, 10],alpha = .2,bins=50, label = "Sum")
