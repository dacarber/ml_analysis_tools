'''
Shower dE/dx using collection plane charge only
'''
...
def compute_shower_dqdx(matched_particles,matched_interaction,input_data, r=10, min_segment_size=3):
    '''
    Inputs:
    ...
    tdrift = 0
    shower_data = {'entry':[], 'voxels':[],'dist':[], 'ind1':[], 'ind2':[], 'coll':[]}
    for ent in range(len(input_data)):
        shower_data['entry'].append(ent)
        for voxel in range(len(input_data[ent])):
            shower_data['voxels'].append(f"{input_data[ent][voxel][1]},{input_data[ent][voxel][2]},{input_data[ent][voxel][3]}")
            shower_data['ind1'].append(input_data[ent][voxel][6])
            shower_data['ind2'].append(input_data[ent][voxel][7])
            shower_data['coll'].append(input_data[ent][voxel][8])
    #print(shower_data['voxels'])
    ...
    for i in range(len(matched_particles)): #Loops over all particles in the matched particle list
        ...
        string_points=[]
        for selp in selected_points:
            string_points.append(f"{selp[0]},{selp[1]},{selp[2]}")
        ...
        if out_of_detector == True:
            continue
        coll = []
        for sp in range(len(selected_points)):
            coll.append(shower_data['coll'][shower_data['voxels'].index(string_points[sp])])
            #print(shower_data['voxels'][shower_data['voxels'].index(string_points[sp])]," string:", string_points[sp])
            
                            
        '''        
        #shower_data['dist'] = cdist(shower_data['voxels'],ppn_prediction.reshape(1,-1))
        #shower_mask = np.zeros((len(shower_data['dist']),1),dtype = bool)
        shower_mask = [False]*len(shower_data['voxels'])
        #shower_mask = np.array(shower_mask)
        print(len(shower_data['dist']))
        voxels = np.array(shower_data['voxels'])
        for sd in range(len(shower_data['voxels'])):
            last = 0
            nex  = 0
            for pp in range(len(selected_points)):
                #print(voxels[sd])
                if (voxels[sd] == selected_points[pp]).all():
                    #print(voxels[sd],selected_points[pp])
                    shower_mask[sd]=True
                    #nex = last+1
            #if nex == last:
            #    shower_mask.append(False)
            #    last = nex
            if sum(shower_mask) == sum(mask):
                break'''
        '''for dis in range(len(dist)):
            spots = np.where(shower_data['dist'] == dist[dis])
            spot = spots[0][0]
            #print(spot)
            shower_mask[spot] = True'''
        #print(shower_data['coll'])
        #shower_mask = np.array(shower_mask)
        if len(coll) != sum(mask):
            print("AHHH",len(coll),sum(mask))
        #coll = np.array(shower_data['coll'])
        dq = sum(coll)#*(23.6*10**(-6))*(86.83)*np.exp(tdrift/3000)*0.66**(-1)
...
for iteration in range(iterations):
    ...
    for entry, index in enumerate(evaluator.index):
        ...
        #print(matched_interaction)
        dqdx = compute_shower_dqdx(matched_particles,matched_interaction,input_data, r=radius, min_segment_size=min_segment_size)
...
pd.DataFrame({'photons':collect_dqdx_photons}).to_csv(f'~/data/data_r{radius}_c2_coll_lane_photons.csv')
pd.DataFrame({'electrons':collect_dqdx_electrons}).to_csv(f'~/data/data_r{radius}_c2_coll_lane_electrons.csv')
pd.DataFrame({'reco':collect_dqdx}).to_csv(f'~/data/data_r{radius}_c2_coll_lane_reco.csv')
pd.DataFrame({'true':collect_dqdx_true}).to_csv(f'~/data/data_r{radius}_c2_coll_lane_true.csv')
