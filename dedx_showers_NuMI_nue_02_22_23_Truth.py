...
def compute_shower_dqdx(true_particles, r=10, min_segment_size=3):
    ...
    for i in range(len(true_particles)): #Loops over all particles in the matched particle list
        ...
        dq_true = np.sum(true_particles[i].depositions[true_mask])*(23.6*10**(-6))*(86.83)*np.exp(tdrift/3000)*0.66**(-1)  # Grabs true dE 
        
        true_total_energy = np.sum(true_particles[i].depositions)*(23.6*10**(-6))*(86.83)*np.exp(tdrift/3000)*0.66**(-1) 
    ...
    return out_true, out_true_photon, out_true_electron, out_true_total,out_true_total_e, out_true_total_p
